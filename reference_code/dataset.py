...
class Dataset:
    def __init__(self, name, args):
        ...
        with open("dataset/split/{}/main_splits.json".format(args.dataset_name), "r") as f:
        ...
        fir, sec = np.random.randint(
            low=0, high=len(self.train_graphs), size=(2, self.args.gen_train_num)
        )
        for i in range(self.args.gen_train_num):
            ...
            match = (
                self.train_graphs[fir[i]].node_features @ self.train_graphs[sec[i]].node_features.T
            )
            normalized_match = F.softmax(match, dim=0)

            mixed_adj = (
                lam * to_dense_adj(self.train_graphs[fir[i]].edge_mat)[0].double()
                + (1 - lam)
                * normalized_match.double()
                @ to_dense_adj(self.train_graphs[sec[i]].edge_mat)[0].double()
                @ normalized_match.double().T
            )
            mixed_adj[mixed_adj < 0.1] = 0
            mixed_x = (
                lam * self.train_graphs[fir[i]].node_features
        ...
        for index in range(self.test_classes_num):
            fir, sec = np.random.randint(
                low=0, high=len(self.test_fine_tune_list), size=(2, self.args.gen_test_num)
            )
            for i in range(self.args.gen_test_num):
                ...
                # emb1 = self.train_graphs[fir[i]].node_features / self.train_graphs[fir[i]].norm(dim=1)
                match = (
                    self.test_fine_tune_list[index][fir[i]].node_features
                    @ self.test_fine_tune_list[index][sec[i]].node_features.T
                )
                normalized_match = F.softmax(match, dim=0)
                mixed_adj = (
                    lam * to_dense_adj(self.test_fine_tune_list[index][fir[i]].edge_mat)[0].double()
                    + (1 - lam)
                    * normalized_match.double()
                ...
                mixed_adj[mixed_adj < 0.1] = 0
                mixed_x = (
                    lam * self.test_fine_tune_list[index][fir[i]].node_features
                    + (1 - lam)
                    * normalized_match.float()
                    @ self.test_fine_tune_list[index][sec[i]].node_features
                )
...
def load_data(dataset, degree_as_tag):
    ...
    if dataset in ["Letter_high", "ENZYMES", "Reddit", "TRIANGLES"]:
        ...
        with open("dataset/datasets/%s/%s.txt" % (dataset, dataset), "r") as f:
            reader = csv.reader(f)
            # print(list(reader))
        ...
        if degree_as_tag:
            for g in g_list:
                ...
                if np.sum(np.array(g.node_tags) == 0):
                    print(g.node_tags)
        ...
        print(dataset)
        node_attribures = pkl.load(
            open("dataset/datasets/{}/{}_node_attributes.pickle".format(dataset, dataset), "rb")
        )
        train_set = pkl.load(
            open("dataset/datasets/{}/{}_train_set.pickle".format(dataset, dataset), "rb")
        )
        val_set = pkl.load(
            open("dataset/datasets/{}/{}_val_set.pickle".format(dataset, dataset), "rb")
        )
        test_set = pkl.load(
            open("dataset/datasets/{}/{}_test_set.pickle".format(dataset, dataset), "rb")
        )
