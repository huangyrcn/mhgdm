...
class Trainer(object):
    ...
    def train_sde(self, ts=None):
        ...
        for epoch in trange(0, (self.config.train.num_epochs), desc = '[Epoch]', position = 1, leave=False):
            ...
            train_pbar = tqdm(enumerate(self.train_loader), desc='[Train]', total=len(self.train_loader), position=2, leave=False)
            for step, train_b in train_pbar:
                x, adj = load_batch(train_b, self.device)
                ...
                train_pbar.set_postfix({
                    'loss_x': f'{loss_x.item():.4e}',
                    'loss_adj': f'{loss_adj.item():.4e}',
                    'avg_x': f'{np.mean(self.train_x):.4e}',
                    'avg_adj': f'{np.mean(self.train_adj):.4e}'
                })
            ...
            self.model_x.eval()
            self.model_adj.eval()
            test_pbar = tqdm(enumerate(self.test_loader), desc='[Test]', total=len(self.test_loader), position=2, leave=False)
            for step, test_b in test_pbar:   
                ...
                test_pbar.set_postfix({
                    'loss_x': f'{loss_x.item():.4e}',
                    'loss_adj': f'{loss_adj.item():.4e}',
                    'avg_x': f'{np.mean(self.test_x):.4e}',
                    'avg_adj': f'{np.mean(self.test_adj):.4e}'
                })
