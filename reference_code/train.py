...
class Trainer:
    def __init__(self, args):
        ...
        self.opt = optim.SGD(
            [{"params": self.log.parameters()}, {"params": self.prompt.parameters()}], lr=0.01
        )
    ...
    def train(self):
        ...
        for i in range(self.epoch_num):
            loss = self.train_one_step(
                mode="train", epoch=i, graph_aug1=graph_aug1, graph_aug2=graph_aug2
            )
            ...
            if i % 50 == 0:
                ...
                if loss < best:
                    ...
                    torch.save(
                        ...
                        "./savepoint/" + self.args.dataset_name + "_model.pkl",
                    )
                else:
                    cnt_wait += 1
    ...
    def test(self):
        ...
        self.model.load_state_dict(
            torch.load("./savepoint/" + self.args.dataset_name + "_model.pkl")
        )
    ...
    def train_one_step(
        self, mode, epoch, graph_aug1=None, graph_aug2=None, test_idx=None, baseline_mode=None
    ):
        if mode == "train":
            self.model.train()
            ...
            support_current_sample_input_embs, support_current_sample_input_embs_selected = (
                self.model.sample_input_GNN([current_task], prompt_embeds, True)
            )  # [N(K+Q), emb_size]
            ...
            if not self.args.test_mixup:
                ...
                data = support_current_sample_input_embs.reshape(
                    self.N_way,
                    self.K_shot + self.args.gen_test_num,
                    self.model.sample_input_emb_size,
                )
                support_data, support_data_mixup = (
                    data[:, : self.K_shot, :]
                    .reshape(self.N_way * self.K_shot, self.model.sample_input_emb_size)
                    .detach(),
                    data[:, self.K_shot : self.K_shot + self.args.gen_test_num, :]
                    .reshape(self.N_way * self.args.gen_test_num, self.model.sample_input_emb_size)
                    .detach(),
                )  # .cpu().numpy()
            ...
            for _ in range(500):
                ...
                loss_mix = (
                    weight * self.xent(logits_mix, support_label_mix_a)
                    + (1 - weight) * self.xent(logits_mix, support_label_mix_b)
                ).mean()
                ...
                if loss_leg < best_loss:
                    ...
                    wait = 0
                    torch.save(
                        self.log.state_dict(), "./savepoint/" + self.args.dataset_name + "_lr.pkl"
                    )
                else:
                    wait += 1
                if wait > patience:
            ...
            self.log.load_state_dict(
                torch.load("./savepoint/" + self.args.dataset_name + "_lr.pkl")
            )
            self.log.eval()
            self.prompt.eval()
            prompt_embeds = self.prompt()
            ...
            if not self.args.test_mixup:
                query_data = (
                    query_current_sample_input_embs.reshape(
                        self.N_way, self.query_size, self.model.sample_input_emb_size
                    )
                    .detach()
                    .cpu()
                    .numpy()
...
def parse_arguments():
    ...
    # GIN parameters
    parser.add_argument("--dataset_name", type=str, default="TRIANGLES", help="name of dataset")

    parser.add_argument("--baseline_mode", type=str, default=None, help="baseline")
...
datasets = ["Letter_high", "ENZYMES", "Reddit", "TRIANGLES"]
# datasets.append(args.dataset_name) # 可以注释掉这一行
res = {}
