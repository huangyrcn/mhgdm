...
from utils.manifolds_utils import proj_tan,is_on_lorentz_manifold
...
def get_dim_act_curv(config, num_layers, enc=True):
    ...
    if enc:
        ...
        dims = [model_config.dim] + [model_config.hidden_dim] * (
            num_layers
        )  # len=args.num_layers+1

    manifold_class = {"PoincareBall": PoincareBall, "Lorentz": Lorentz}

    if enc:
        manifolds = [
            manifold_class[model_config.manifold](
                model_config.c, learnable=model_config.learnable_c
            )
...
class HNNLayer(nn.Module):
    ...
    def __init__(
        self, in_dim, out_dim, manifold_in, manifold_out, dropout=0.0, act=nn.ReLU(), use_norm=True
    ):
...
class HGATLayer(nn.Module):
    ...
    def forward(self, input):
        ...
        x = self.dropout(x)
        nodes_features_proj = self.linear_proj(x).view(b, n, self.num_of_heads, -1)
        ...
        score = self.scoring_fn(
            torch.cat(
                [x_left, x_right, adj[..., None, None].expand(-1, -1, -1, self.num_of_heads, -1)],
                dim=-1,
            )
        ).squeeze()
        ...
        if self.local_agg:
            ...
            score = score + connectivity_mask.unsqueeze(-1).expand(
                -1, -1, -1, self.num_of_heads
            )  # (b,n,n,n_head) padding的地方会-9e15
        ...
        # addend = F.normalize(addend, dim=-1) * 0.1  
...
class HypAgg(Module):
    ...
    def forward(self, x, adj):
        ...
        att = self.att_net(
            self.manifold.logmap0(x_left),
            self.manifold.logmap0(x_right),
            adj.unsqueeze(-1),
            edge_attr,
        )  # (b,n_node,n_node,dim)
        if self.msg_transform:
            ...
            msg = self.manifold.logmap(
                x_left, x_right
            )  # (b,n_node,n_node,dim)  x_col落在x_row的切空间
        msg = msg * att
