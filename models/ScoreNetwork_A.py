...
class ScoreNetworkA_poincare(BaselineNetwork):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(ScoreNetworkA_poincare, self).__init__(
            max_feat_num,
            max_node_num,
            nhid,
            num_layers,
        ...
        self.time_scale = nn.Sequential(
            nn.Linear(self.nfeat, self.nfeat), nn.ReLU(), nn.Linear(self.nfeat, 1)
        )
        self.mask.unsqueeze_(0)
    ...
    def forward(self, x, adj, flags, t):
        ...
        out_shape = adjs.shape[:-1]  # B x N x N
        time_scale_factor = self.time_scale(temb)  # [B, 1]
        # 扩展时间缩放因子以匹配score的形状 [B, N, N]
        time_scale_factor = time_scale_factor.view(-1, 1, 1)  # [B, 1, 1]
        score = self.final(adjs).view(*out_shape) * time_scale_factor
        self.mask = self.mask.to(score.device)
        score = score * self.mask
        score = mask_adjs(score, flags)
        return score
...
class HScoreNetworkA(BaselineNetwork):
    ...
    def __init__(
        ...
        super(HScoreNetworkA, self).__init__(
            max_feat_num,
            max_node_num,
            nhid,
            num_layers,
