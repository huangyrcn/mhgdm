    ...
    def update_fn(self, x, adj, flags, t):
        ...
        if self.obj == "x":
            ...
            if self.sde.hyp:  # 如果使用双曲空间
                ...
                x = exp_after_transp0(
                    x_mean, diffusion * np.sqrt(-dt) * z, m
                )  # 添加噪声 (平行传输后)
            else:  # 欧氏空间
        ...
        if self.obj == "x":
            ...
            for i in range(n_steps):  # 执行 n_steps 校正
                ...
                if sde.hyp:  # 如果使用双曲空间
                    ...
                    x = exp_after_transp0(
                        x_mean, torch.sqrt(step_size * 2) * noise * seps, m
                    )  # 添加噪声 (平行传输后)
                else:  # 欧氏空间
...
def get_pc_sampler(
    ...
    def pc_sampler(model_x, model_adj, shape_x, shape_adj):
        # 获取配置参数
        ...
        with torch.no_grad():  # 禁用梯度计算
            ...
            for i in trange(
                ...
                # 校正步骤
                x, x_mean = corrector_obj_x.update_fn(x, adj, flags, vec_t)  # 校正 x
                adj, adj_mean = corrector_obj_adj.update_fn(_x, adj, flags, vec_t)  # 校正 adj
