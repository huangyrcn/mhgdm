...
def data_log(logger, config):
    logger.log(
        f"[{config.data.name}]   init={config.data.init} ({config.data.max_feat_num})   seed={config.seed}   batch_size={config.data.batch_size}"
    )
...
def sample_log(logger, config):
    ...
    if config.sampler.corrector == "Langevin":
        sample_log += (
            f"|| snr={config.sampler.snr_x} seps={config.sampler.scale_eps_x} "
