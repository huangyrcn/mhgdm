...
class Sampler_mol(object):
    ...
    def sample(self, independent=True):
        ...
        if self.config.data.name == "ZINC250k":
            n_samples = 10000
            batch_size = 5000
        else:
            n_samples = 10000
        ...
        self.sampling_fn = load_sampling_fn(
            self.configt, self.config.sampler, self.config.sample, self.device, manifold, batch_size
        )
        ...
        train_smiles, test_smiles = load_smiles(self.configt.data.name)
        train_smiles, test_smiles = canonicalize_smiles(train_smiles), canonicalize_smiles(
            test_smiles
        )

        self.train_graph_list, _ = load_data(self.configt, get_graph_list=True)  # for init_flags
        with open(f"data/{self.configt.data.name.lower()}_test_nx.pkl", "rb") as f:
        ...
        for r in range(n_iter):
            self.init_flags = init_flags(self.train_graph_list, self.configt, batch_size).to(
                self.device
            )
            x, adj = self.sampling_fn(self.model_x, self.model_adj, self.init_flags)
            ...
            adj = torch.nn.functional.one_hot(torch.tensor(samples_int), num_classes=4).permute(
                0, 3, 1, 2
            )
            gen_mols, num_mols_wo_correction = gen_mol(x, adj, self.configt.data.name)
            ...
            wandb.log(
                {
                    ...
                    "validity w/o correction": num_mols_wo_correction_total / num_mols,
                    "valid": metric_total["valid"],
                    f"unique@{gen_smiles_len}": metric_total[f"unique@{gen_smiles_len}"],
                    "FCD/Test": metric_total["FCD/Test"],
                    "Novelty": metric_total["Novelty"],
